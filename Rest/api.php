	...
	class API extends REST {
		...
		*/
		...
		private function customers(){	
			...
			$query="SELECT distinct c.numeroCliente, c.nombreCliente, c.email, c.direccion, c.ciudad, c.provincia, c.codigoPostal, c.pais FROM clientes c order by c.numeroCliente desc";
			$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
		...
		private function customer(){	
			...
			if($id > 0){	
				$query="SELECT distinct c.numeroCliente, c.nombreCliente, c.email, c.direccion, c.ciudad, c.provincia, c.codigoPostal, c.pais FROM clientes c where c.numeroCliente=$id";
				$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
				if($r->num_rows > 0) {
					$result = $r->fetch_assoc();	
					$this->response($this->json($result), 200); // send user details
		...
		private function insertCustomer(){
			...
			$column_names = array('nombreCliente', 'email', 'ciudad', 'direccion', 'pais');
			...
			$query = "INSERT INTO clientes(".trim($columns,',').") VALUES(".trim($values,',').")";
			if(!empty($customer)){
				$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
